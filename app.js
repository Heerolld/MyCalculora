...
buttons.forEach((btn) => {
    btn.addEventListener("click", () => {
        ...
        if (!btn.id.match('erase')) {
            ...
            if (btn.id === 'invert-sign') {
                if (realTimeScreenValue.length > 0) {
                    // Verifica se o último caractere é um sinal negativo
                    if (realTimeScreenValue[0] === '-') {
                        realTimeScreenValue.shift(); // Remove o sinal negativo
                    } else {
                        realTimeScreenValue.unshift('-'); // Adiciona um sinal negativo no início
                    }
